...
HEADER_TRYLON_BLOCKED = "X-Trylon-Blocked"
HEADER_TRYLON_SAFETY_CODE = "X-Trylon-Safety-Code"
HEADER_TRYLON_ACTION = "X-Trylon-Action"
HEADER_TRYLON_MESSAGE = "X-Trylon-Message"
...
def _create_openai_blocked_response_body(
    ...
    response_id = f"trylon-blocked-{int(time.time())}"
    ...
    return {
        ...
        "usage": usage,
        "system_fingerprint": "fp_trylon_blocked",
    }
...
def create_blocked_response(
    ...
    Determines the final HTTP status code based on the block_status:
        ...
        - Returns 200 OK for standard policy violations (e.g., PII, Profanity)
          detected by Trylon guardrails, mimicking native filter behavior.
        - Returns 500 for unsupported providers.

    Includes standard 'X-Trylon-*' headers in the response.
