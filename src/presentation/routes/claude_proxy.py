...
def _extract_input_messages_from_claude(
    ...
    """
    Converts Claude's 'messages' structure to Trylon's message list format.
...
async def claude_messages_proxy(
    ...
    try:
        ...
        raise client_err
    except TrylonHTTPException as http_exc:
        logger.warning(
            f"TrylonHTTPException in Claude proxy route: {http_exc.detail} (Status: {http_exc.status_code})",
            extra=log_extra,
            exc_info=False,
        )
        raise http_exc
