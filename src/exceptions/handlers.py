...
async def trylon_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Handler for all TrylonBaseError exceptions."""
    trylon_exc = exc if isinstance(exc, TrylonBaseError) else None
    if not trylon_exc:
...
async def http_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Handler for TrylonHTTPException (usually raised intentionally in routes)."""
    http_exc = exc if isinstance(exc, TrylonHTTPException) else None
    if not http_exc:
        return await unhandled_exception_handler(request, exc)
