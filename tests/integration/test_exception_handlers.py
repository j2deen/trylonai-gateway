...
def app_with_handlers():
    ...
    @app.get("/trylon-base-500")
    async def raise_trylon_base_500():
        raise TrylonBaseError(
            "Base 500 (Internal)", status.HTTP_500_INTERNAL_SERVER_ERROR
        )

    @app.get("/trylon-base-400-user")
    async def raise_trylon_base_400_user():
        raise TrylonBaseError(
            "Base 400 (User)", status.HTTP_400_BAD_REQUEST, user_facing=True
        )

    @app.get("/trylon-base-400-internal")
    async def raise_trylon_base_400_internal():
        raise TrylonBaseError(
    ...
    @app.get("/trylon-http-exception")
    async def raise_trylon_http():
        raise TrylonHTTPException(
...
def test_trylon_base_500_handler(client: TestClient):
    response = client.get("/trylon-base-500")
...
def test_trylon_base_400_user_handler(client: TestClient):
    response = client.get("/trylon-base-400-user")
...
def test_trylon_base_400_internal_handler(client: TestClient):
    response = client.get("/trylon-base-400-internal")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    data = response.json()
    assert data["safety_code"] == SafetyCode.GENERIC_UNSAFE
    assert data["message"] == "An unexpected error occurred. Please try again later."
...
def test_trylon_http_exception_handler(client: TestClient):
    response = client.get("/trylon-http-exception")
