...
def test_trylon_base_error():
    """Test that TrylonBaseError initializes correctly."""
    error = TrylonBaseError("Base error", status.HTTP_500_INTERNAL_SERVER_ERROR)
    assert error.message == "Base error"
    assert error.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    assert error.user_facing is False
    assert str(error) == "Base error"

    error_user = TrylonBaseError(
...
def test_trylon_http_exception():
    """Test TrylonHTTPException initialization."""
    error = TrylonHTTPException(
        status_code=418,
    ...
    assert not isinstance(error, TrylonBaseError)
