...
class TestGeminiProxy:
    ...
    def test_gemini_proxy_success(
        ...
        assert HEADER_TRYLON_BLOCKED not in response.headers
    ...
    def test_gemini_proxy_input_blocked(
        ...
        assert response.headers.get(HEADER_TRYLON_BLOCKED) == "true"
        assert response.headers.get(HEADER_TRYLON_SAFETY_CODE) == str(
            UNSAFE_STATUS_BLOCK_FROM_VALIDATOR.safety_code
        )
        assert response.headers.get(HEADER_TRYLON_ACTION) == str(
            UNSAFE_STATUS_BLOCK_FROM_VALIDATOR.action
        )
        assert (
            response.headers.get(HEADER_TRYLON_MESSAGE)
            == UNSAFE_STATUS_BLOCK_FROM_VALIDATOR.message
        )
    ...
    def test_gemini_proxy_output_blocked(
        ...
        fake_headers = {
            ...
            HEADER_TRYLON_SAFETY_CODE: str(
                UNSAFE_STATUS_BLOCK_FROM_VALIDATOR.safety_code
            ),
            HEADER_TRYLON_ACTION: str(UNSAFE_STATUS_BLOCK_FROM_VALIDATOR.action),
            HEADER_TRYLON_MESSAGE: UNSAFE_STATUS_BLOCK_FROM_VALIDATOR.message,
        }
