...
// Configuration
const TRYLON_GATEWAY_URL = 'http://localhost:8000';
...
async function runSafeguardExample() {
  ...
  try {
    const response = await axios.post(`${TRYLON_GATEWAY_URL}/safeguard`, safePayload);
  ...
  try {
    const response = await axios.post(`${TRYLON_GATEWAY_URL}/safeguard`, piiPayload);
    console.log(`Status Code: ${response.status}`);
    console.log('Response JSON:', JSON.stringify(response.data, null, 2));
