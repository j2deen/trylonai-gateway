...
def run_openai_proxy_examples():
    """Demonstrates using the Trylon Gateway as an OpenAI proxy."""
    ...
    try:
        ...
        is_blocked = response_with_raw.headers.get("X-Trylon-Blocked")
        safety_code = response_with_raw.headers.get("X-Trylon-Safety-Code")

        print(f"Header 'X-Trylon-Blocked': {is_blocked}")
        print(f"Header 'X-Trylon-Safety-Code': {safety_code}")
        print(
            "--> Expected: finish_reason: 'content_filter' and X-Trylon-Blocked: 'true'"
        )
