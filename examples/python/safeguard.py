...
TRYLON_GATEWAY_URL = "http://localhost:8000"
...
def run_safeguard_example():
    ...
    response = requests.post(f"{TRYLON_GATEWAY_URL}/safeguard", json=safe_payload)
    ...
    response = requests.post(f"{TRYLON_GATEWAY_URL}/safeguard", json=pii_payload)
    print(f"Status Code: {response.status_code}")
    print("Response JSON:", json.dumps(response.json(), indent=2))
